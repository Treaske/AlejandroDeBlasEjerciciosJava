...
public class PartidoJpaController implements Serializable {
    ...
    public PartidoJpaController() {
        emf = Persistence.createEntityManagerFactory("partidosPU");
    }
    ...
    public Partido findPartidoNombre(String nombre) {
        EntityManager em = getEntityManager();
        try {
            TypedQuery<Partido> query = em.createQuery("SELECT p FROM Partido p WHERE p.nombre = :nombre", Partido.class);
